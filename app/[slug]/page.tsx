    ...
    <>
      ...
      <main className="min-h-screen brut-page flex flex-col">
        ...
        <div className="w-full max-w-5xl mx-auto px-4 pt-6 sm:pt-8 pb-1 sm:pb-2">
          {/* Breadcrumb */}
          <div className="flex items-center gap-2 text-sm text-ink/60 mb-4 sm:mb-6 font-bold">
            <Link href="/" className="hover:text-brut-pink transition-colors">{homeBreadcrumbName}</Link>
            <span>›</span>
            <Link href="/apps" className="hover:text-brut-pink transition-colors">{appsBreadcrumbName}</Link>
            <span>›</span>
            <span className="text-ink">{breadcrumbName}</span>
          </div>

          <div className="text-center">
            <h1 className="brut-display text-2xl sm:text-3xl md:text-4xl text-ink leading-tight">
              {post.title}
            </h1>
            {post.description && (
              <p className="text-ink/70 text-base sm:text-lg font-semibold mt-3 max-w-3xl mx-auto leading-relaxed">
                {post.description}
              </p>
            )}
          </div>
        ...
        <div className="w-full flex-grow">
          <div className="w-full max-w-5xl mx-auto px-4 pt-1 sm:pt-2 pb-10 sm:pb-16">
            ...
            {/* Kapak Fotoğrafı */}
            {post.coverImage ? (
              <div className="w-full overflow-hidden mb-4 sm:mb-8 border-3 border-ink" style={{ borderWidth: 3 }}>
                <img
                  src={post.coverImage}
                  alt={post.title}
                  loading="lazy"
              ...
              <div className="w-full h-[200px] sm:h-[360px] overflow-hidden mb-4 sm:mb-8 border-3 border-ink bg-gradient-to-tr from-blue-500 to-teal-400 flex flex-col items-center justify-center" style={{ borderWidth: 3 }}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
              ...
              <aside className="order-1 lg:order-2 lg:col-span-1">
                {/* Not: sticky wrapper kasıtlı olarak stil taşımıyor — transform/filter içeren
                    bir sınıfla aynı elemana konursa position:sticky tarayıcıda bozuluyor. */}
                <div className="lg:sticky lg:top-8">
                <div className="brut-card-static p-4 sm:p-6">

                  {/* Yazar (+ mobilde sağda kompakt yıldız/oy) */}
                  <div className="flex items-center justify-between gap-3 pb-2 mb-2 border-b-2 border-ink/15">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-11 h-11 rounded-full overflow-hidden shrink-0 border-2 border-ink bg-paper">
                        {post.authorImage ? (
                          <img
                          ...
                          <div className="w-full h-full flex items-center justify-center bg-paper text-ink/40">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs text-ink/50 font-semibold">Written by</p>
                        <p className="text-ink font-black text-sm truncate">{post.author || 'Omegle Test'}</p>
                      </div>
                    </div>
                    ...
                    <div className="text-right shrink-0 sm:hidden">
                      <div className="flex items-center justify-end gap-0.5">
                        {[...Array(5)].map((_, i) => (
                          <svg key={i} className={`w-3.5 h-3.5 ${i < Math.round(rating) ? 'text-brut-yellow' : 'text-ink/15'}`} fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                          </svg>
                        ))}
                        <span className="text-ink font-black text-sm ml-1">{rating.toFixed(1)}</span>
                      </div>
                      <p className="text-xs text-ink/50 font-semibold">{voteCount.toLocaleString('en-US')} votes</p>
                    </div>
                  </div>
                  ...
                  <div className="hidden sm:block">
                    <div className="flex items-center gap-1 mb-1">
                      {[...Array(5)].map((_, i) => (
                        <svg key={i} className={`w-5 h-5 ${i < Math.round(rating) ? 'text-brut-yellow' : 'text-ink/15'}`} fill="currentColor" viewBox="0 0 20 20">
                          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                        </svg>
                      ))}
                      <span className="text-ink font-black ml-1">{rating.toFixed(1)}</span>
                    </div>
                    <p className="text-sm text-ink/50 font-semibold mb-2">{voteCount.toLocaleString('en-US')} votes</p>
                  </div>

                  {/* Tarih — masaüstü (ikonlu, iki satır) */}
                  <div className="hidden sm:flex items-center gap-3 pt-2 border-t-2 border-ink/15">
                    <div className="w-10 h-10 bg-brut-yellow/30 text-ink border-2 border-ink flex items-center justify-center shrink-0">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-xs text-ink/50 font-semibold">{dateLabel}</p>
                  ...
                  <p className="sm:hidden text-center text-xs text-ink/50 font-semibold pt-2 border-t-2 border-ink/15">
                    {dateLabel}: <span className="text-ink font-black">{displayDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'numeric', year: 'numeric' })}</span>
                  </p>
                  ...
                  <Link
                    id="hero-video-cta"
                    href="/live-video"
                    className="brut-btn mt-3 w-full inline-flex items-center justify-center gap-2 bg-brut-green text-ink font-bold py-3 px-4"
                  >
              ...
              <div className="order-2 lg:order-1 lg:col-span-2 min-w-0 space-y-4 sm:space-y-6">
                ...
                {/* Asıl Blog İçeriği */}
                {post.content && (
                  <div className="relative brut-card-static p-4 sm:p-6 w-full overflow-hidden">
                    <div
                      className="relative text-sm sm:text-base text-ink/80 font-medium leading-relaxed break-words
                      [&>p]:mb-4 sm:[&>p]:mb-6 [&>p:empty]:min-h-[1.5rem]
                      [&>h1]:text-xl sm:[&>h1]:text-3xl [&>h1]:font-black [&>h1]:text-ink [&>h1]:mb-3 sm:[&>h1]:mb-4 [&>h1]:mt-6 sm:[&>h1]:mt-8
                      [&>h2]:text-lg sm:[&>h2]:text-2xl [&>h2]:font-black [&>h2]:text-ink [&>h2]:mb-3 sm:[&>h2]:mb-4 [&>h2]:mt-6 sm:[&>h2]:mt-8
                      [&>h3]:text-base sm:[&>h3]:text-xl [&>h3]:font-black [&>h3]:text-ink [&>h3]:mb-2 sm:[&>h3]:mb-3 [&>h3]:mt-5 sm:[&>h3]:mt-6
                      [&>ul]:list-disc [&>ul]:pl-6 [&>ul]:mb-4 sm:[&>ul]:mb-6
                      [&>ol]:list-decimal [&>ol]:pl-6 [&>ol]:mb-4 sm:[&>ol]:mb-6
                      [&>strong]:font-black [&>strong]:text-ink [&>em]:italic
                      [&_a]:text-brut-pink [&_a]:underline [&_a]:font-bold
                      [&_table]:w-full [&_table]:border-collapse [&_table]:border-2 [&_table]:border-ink [&_table]:mb-6
                      [&_th]:border-2 [&_th]:border-ink [&_th]:bg-brut-yellow/30 [&_th]:text-ink [&_th]:p-3 [&_th]:font-black [&_th]:text-left
                      [&_td]:border-2 [&_td]:border-ink [&_td]:p-3
                ...
                {(pros.length > 0 || cons.length > 0) && (
                  <div className="brut-card-static p-4 sm:p-6">
                    <h3 className="brut-display text-lg sm:text-2xl text-ink mb-4 sm:mb-6">Pros & Cons</h3>
                    <div className="border-2 border-ink overflow-hidden">
                      <table className="w-full text-left border-collapse table-fixed">
                        <thead>
                          <tr className="bg-brut-yellow/30 border-b-2 border-ink">
                            <th className="py-1.5 px-2.5 sm:py-2 sm:px-4 text-xs sm:text-sm font-black text-brut-green uppercase tracking-wide w-1/2 border-r-2 border-ink">Pros</th>
                            <th className="py-1.5 px-2.5 sm:py-2 sm:px-4 text-xs sm:text-sm font-black text-brut-red uppercase tracking-wide w-1/2">Cons</th>
                          </tr>
                        </thead>
                        <tbody>
                          {Array.from({ length: maxRows }).map((_, index) => (
                            <tr key={index} className="border-b border-ink/15 last:border-0">
                              <td className="py-[5px] px-2.5 sm:py-[7px] sm:px-4 align-top border-r-2 border-ink">
                                {pros[index] && (
                                  <span className="block text-ink/80 font-medium text-sm sm:text-base leading-normal sm:text-justify">{pros[index]}</span>
                                )}
                              </td>
                              <td className="py-[5px] px-2.5 sm:py-[7px] sm:px-4 align-top">
                                {cons[index] && (
                                  <span className="block text-ink/80 font-medium text-sm sm:text-base leading-normal sm:text-justify">{cons[index]}</span>
                                )}
                              </td>
                            </tr>
                          ))}
                ...
                {post.alternativeAppsContent && (
                  <div className="brut-card-static p-4 sm:p-6">
                    <h3 className="brut-display text-lg sm:text-2xl text-ink mb-4 sm:mb-6">Alternative Applications</h3>
                    <div
                      className="text-sm sm:text-base text-ink/80 font-medium leading-relaxed break-words
                      [&>p]:mb-3 sm:[&>p]:mb-4 [&>p:empty]:min-h-[1.5rem]
                      [&>h1]:text-lg sm:[&>h1]:text-2xl [&>h1]:font-black [&>h1]:text-ink [&>h1]:mb-3 sm:[&>h1]:mb-4
                      [&>h2]:text-base sm:[&>h2]:text-xl [&>h2]:font-black [&>h2]:text-ink [&>h2]:mb-2 sm:[&>h2]:mb-3
                      [&>h3]:text-sm sm:[&>h3]:text-lg [&>h3]:font-black [&>h3]:text-ink [&>h3]:mb-2
                ...
                {post.faqs && post.faqs.length > 0 && (
                  <div className="brut-card-static p-4 sm:p-6">
                    <h3 className="brut-display text-lg sm:text-2xl text-ink mb-4 sm:mb-6">Frequently Asked Questions</h3>
                      ...
                      {post.faqs.map((faq: PostFaq, index: number) => (
                        ...
                        >
                          <summary className="flex cursor-pointer items-center justify-between px-4 sm:px-6 py-3 sm:py-4 focus:outline-none">
                            <span className="font-bold text-ink text-sm sm:text-base pr-4">{faq.question}</span>
                            <span className="shrink-0 w-7 h-7 sm:w-8 sm:h-8 brut-sticker bg-brut-yellow text-ink flex items-center justify-center transition-transform duration-300 group-open:rotate-45">
                          ...
                          <div className="px-4 sm:px-6 pb-4 sm:pb-5 pt-0 text-ink/70 font-semibold text-xs sm:text-sm leading-relaxed border-t-2 border-ink/15">
                            {faq.answer}
                          </div>
                        </details>
                      ))}
