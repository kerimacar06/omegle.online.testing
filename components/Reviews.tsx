    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-8 sm:py-16">
      ...
      <div className="flex flex-col items-center justify-center mb-10 gap-2">
        <div className="text-center">
          ...
          <p className="text-ink/70 text-xs sm:text-lg font-semibold leading-none">Don&apos;t just take our word for it.</p>
        </div>
      </div>
      ...
      <div className="relative w-full flex items-center group">
        ...
        <button
          ...
          className="brut-btn absolute left-1 md:left-0 z-10 md:-ml-12 w-8 h-8 md:w-12 md:h-12 rounded-full bg-surface flex items-center justify-center text-ink touch-manipulation"
        ...
        >
          {infiniteReviews.map((review, index) => {
            return (
              ...
              >
                <div className="brut-card-static w-full p-6 md:p-8 flex flex-col mx-auto h-full">
                  {/* Üst Kısım: Avatar, İsim ve @Username */}
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 md:w-16 md:h-16 rounded-full overflow-hidden shrink-0 border-2 border-ink">
                        <img src={review.avatar} alt={review.name} className="w-full h-full object-cover" />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-ink font-black text-[16px] md:text-[18px] leading-tight">
                          {review.name}
                        </span>
                        <span className="text-ink/50 font-semibold text-[14px] md:text-[15px]">
                          {review.username}
                        </span>
                      </div>
                    </div>
                  ...
                  {/* Yorum İçeriği */}
                  <p className="text-ink/80 text-xs md:text-[16px] leading-relaxed mt-2 font-semibold md:text-justify line-clamp-5 min-h-[76px] md:min-h-[130px]">
                    &quot;{review.text}&quot;
                  </p>

                  {/* Alt Kısım: Yıldızlar */}
                  <div className="mt-0 md:mt-3 flex items-center justify-start border-t-2 border-ink/15 pt-0.5 md:pt-3">
                    <div className="flex items-center gap-1">
                      {[...Array(review.rating)].map((_, i) => (
                        <svg key={i} className="w-5 h-5 text-brut-yellow" fill="currentColor" viewBox="0 0 20 20">
        ...
        <button
          ...
          className="brut-btn absolute right-1 md:right-0 z-10 md:-mr-12 w-8 h-8 md:w-12 md:h-12 rounded-full bg-surface flex items-center justify-center text-ink touch-manipulation"
